...
int bezopasnyVvodChisla(int minZnach, int maxZnach) {
    ...
    while (true) {
        ...
        if (cin.fail() || chislo < minZnach || chislo > maxZnach) {
            ...
            cout << "\t" << Color::red << "ќшибка! " << Color::yellow << "¬ведите число от " << minZnach << " до " << maxZnach << ": " << Color::reset;
        }
        else {
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            return chislo;
...
int sdelatStavku(int fichki, int minStavka) {
    while (true) {
        ...
        if (cin.fail()) {
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            cout << "\n\t" << Color::red << "ќшибка: введите число." << Color::reset << "\n";
            continue;
        }
        ...
        if (stavka > fichki) {
            cout << "\n\t" << Color::yellow << "” вас только " << fichki << " фишек!" << Color::reset << "\n";
            continue;
        }
